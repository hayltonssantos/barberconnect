...
// Configuração dos plugins do dayjs
dayjs.extend(utc);
...
interface DateContextType {
  ...
  toISOString: (date: Dayjs) => string;
  fromISOString: (isoString: string) => Dayjs;
...
const DateProvider = ({ 
  ...
  const ensureDayjs = (date: Dayjs | Date | string): Dayjs => {
  ...
  // Funções de conversão
  const toISOString = useCallback((date: Dayjs): string => {
    return date.toISOString();
  }, []);

  const fromISOString = useCallback((isoString: string): Dayjs => {
    return dayjs(isoString);
  }, []);

  const toDate = useCallback((date: Dayjs): Date => {
    return date.toDate();
  }, []);

  const fromDate = useCallback((date: Date): Dayjs => {
    return dayjs(date);
  }, []);
...
export { DateContext, DateProvider, useDate };
